	...
	private void doPay(JSONArray args, final CallbackContext callbackContext) {
		...
		try {
			...
			callbackContext.sendPluginResult(
					new PluginResult(PluginResult.Status.ERROR, "Error 0x00000: Invalid arguments: " + e.getCause()));
			return;
		}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (data == null) {
			callback.sendPluginResult(new PluginResult(PluginResult.Status.ERROR,
					getErrorMessage(requestCode, resultCode, "Action cancelled")));
			return;
		}
		...
		if (code != SumUpAPI.Response.ResultCode.SUCCESSFUL) {
			...
			callback.sendPluginResult(
					new PluginResult(PluginResult.Status.ERROR, getErrorMessage(requestCode, code, message)));
			return;
		}
